Ext.BLANK_IMAGE_URL = '../js/ext/frame/s.gif';
...
MainPanel = function(){
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        items: {
            id:'welcome-panel',
            title: '系统首页',
           // autoLoad: {url: 'welcome.php', callback: this.initSearch, scope: this},
...
Ext.extend(MainPanel, Ext.TabPanel, {
    ...
    onClick: function(e, target){
        ...
        if(target = e.getTarget('a:not(.exi)', 3)){
            ...
            if(cls){
                ...
                var tabtext = target.innerHTML;
                this.loadClass(target.href, tabtext, '#');
            }
        ...
        /*
        if(target = e.getTarget('input:not(.exi)', 3)){
        		var iType = Ext.fly(target).getAttributeNS('', 'type');
    ...
    loadClass : function(href, cls, member){
        ...
        var iframeId = "ifr-" + cls;
        
        var iframeHtml = "<iframe id='" + iframeId + "' src='"  + href + "' width='100%' height='100%'></iframe>";
        if(tab){
            this.setActiveTab(tab);
            
            var p2 = this.getActiveTab();
            ...
            if (p2) {
         			document.frames(iframeId).location.href = href;         		
         		}
            
        }else{
            var autoLoad = {url: href,scope: this};
            if(member){
        ...
        new Ext.Toolbar({
			...
			}), ' ', ' ',
			{
                ...
                toggleHandler : function(b, pressed){
                     ...
                     window.location.href = 'login.jsp?task=logout';
                }
            }]
        })]
    })
...
//showWindow("http://www.tnc.com.cn","tnc") ;
...
//另外的窗口显示
function showWindow(desUrl,title) 
{
	theMainPanel.panel.loadClass(desUrl,title);
}